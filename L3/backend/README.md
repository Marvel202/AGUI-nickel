...
The agent uses `mistral-large-latest` with a `query_data` tool that loads rows
from `db.csv`. Ensure `MISTRAL_API_KEY` is set (for example in the repo root
`.env`, which the helper loads automatically).
